...
namespace mfem_mgis {
  ...
  IsotropicTridimensionalStationaryNonLinearHeatTransferBehaviourIntegrator::
    ...
    const auto dNi_1 = dN(ni, 1);
    const auto dNi_2 = dN(ni, 2);
    Fe[ni] += w * (s[2] * dNi_2 + dNi_1 * s[1] + s[0] * dNi_0);
  ...
  IsotropicTridimensionalStationaryNonLinearHeatTransferBehaviourIntegrator::
    ...
    for (size_type nj = 0; nj != nnodes; ++nj) {
      ...
      Ke(ni, nj) +=
          w * (Kip[5] * dNj_2 * dNi_1 + dNj_0 * Kip[3] * dNi_1 +
               dNj_2 * dNi_2 * Kip[8] + dNi_2 * dNj_0 * Kip[6] +
               dNi_0 * dNj_1 * Kip[1] + dNj_0 * Kip[0] * dNi_0 +
               dNi_2 * dNj_1 * Kip[7] + Kip[2] * dNj_2 * dNi_0 +
               dNj_1 * Kip[4] * dNi_1 +
               (Kip[9] * dNi_0 + Kip[10] * dNi_1 + Kip[11] * dNi_2) * N[nj]);
