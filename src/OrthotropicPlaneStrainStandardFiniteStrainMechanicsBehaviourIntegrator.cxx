...
namespace mfem_mgis {
  ...
  OrthotropicPlaneStrainStandardFiniteStrainMechanicsBehaviourIntegrator::
    ...
    for (size_type nj = 0; nj != nnodes; ++nj) {
      ...
      Ke(ni_0, nj_1) += w * (dNj_1 * Kip[1] * dNi_0 + dNi_1 * Kip[16] * dNj_1 +
                             dNj_0 * Kip[4] * dNi_0 + dNi_1 * dNj_0 * Kip[19]);
      Ke(ni_1, nj_0) += w * (dNi_1 * dNj_0 * Kip[5] + dNj_0 * dNi_0 * Kip[20] +
                             dNj_1 * Kip[23] * dNi_0 + dNi_1 * dNj_1 * Kip[8]);
      Ke(ni_1, nj_1) += w * (Kip[21] * dNj_1 * dNi_0 + dNi_1 * Kip[6] * dNj_1 +
