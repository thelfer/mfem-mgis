...
namespace mfem_mgis {
  ...
  bool DependenciesManager::declareDependency(Context &ctx,
    ...
    const auto pd = [this, &nd, &ldm] {
      ...
      if (isValid(qspace)) {
        return std::find_if(
            ldm.begin(), ldm.end(), [this, &nd, qspace](const auto &dep) {  //
  ...
  bool DependenciesManager::setProvider(
      ...
      if (d.getName() == ld.getName()) {
        ...
        if ((isValid(oldqspace)) && (isValid(dqspace))) {
          if (this->qids.areEquivalent(oldqspace, qspace)) {
            ctx.assertOrTerminate(pd == ldm.end(),
                                  "internal error: " + d.getDescription() +
                                      " is registered twice");
          ...
          ctx.assertOrTerminate(
              pd == ldm.end(),
              "internal error: " + d.getDescription() + " is registered twice");
          if (!ld.setProvider(ctx, pr)) {
            return false;
          }
          pd = p;
    ...
    if (isInvalid(dqspace)) {
      ...
      for (auto p = ldm.begin(); p != ldm.end(); ++p) {
        ...
        ctx.assertOrTerminate(
            isValid(p->getPartialQuadratureSpace()),
            "internal error: " + pd->getDescription() + " declared twice");
