...
namespace mfem_mgis {
  ...
  struct MFEM_MGIS_EXPORT AbstractCouplingScheme : AbstractCouplingItem {
     ...
     * The key gives the type of the item inserted (for example, 'Model' or
     * 'model'). The value must be dictionary describing the item. This
     * description must be a dictionary with one element. The first element must
     * be the name of the item and the value must a dictionary that describe the
     ...
     */
    [[nodiscard]] virtual bool add(Context &, const Parameters &) noexcept = 0;
    /*!
     * \brief add a new coupling item
     *
