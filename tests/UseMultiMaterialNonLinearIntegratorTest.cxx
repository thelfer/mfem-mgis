/*!
 * \file   tests/UseMultiMaterialNonLinearIntegratorTest.cxx
 * \brief  This test checks that the `UseMultiMaterialNonLinearIntegrator`
 * parameter of a non linear evolution problem is taken into account.
 * \date   24/09/2026
 */
...
struct UseMultiMaterialNonLinearIntegratorTest final
    : public tfel::tests::TestCase {
  UseMultiMaterialNonLinearIntegratorTest()
      : tfel::tests::TestCase("MFEMMGIS",
                              "UseMultiMaterialNonLinearIntegratorTest") {
  }  // end of UseMultiMaterialNonLinearIntegratorTest
 ...
 private:
  ...
  //! \brief the multi-material integrator is used by default
  void test1() {
    ...
    TFEL_TESTS_CHECK(
        isValid(oproblem->getNumberOfBehaviourIntegrators(ctx, 1)));
  }
  //! \brief the multi-material integrator can be disabled
  void test2() {
    using namespace mfem_mgis;
    auto ctx = Context{};
    ...
    TFEL_TESTS_ASSERT(isValid(oproblem));
    TFEL_TESTS_CHECK(
        isInvalid(oproblem->getNumberOfBehaviourIntegrators(ctx, 1)));
    TFEL_TESTS_CHECK(ctx.getRawErrorMessage() ==
                     "support for mgis integrator has been disabled");
  }
  //! \brief a value of the wrong type is reported
  void test3() {
    using namespace mfem_mgis;
    auto ctx = Context{};
    auto params = getProblemParameters();
    params.insert(throwing, "UseMultiMaterialNonLinearIntegrator", 1);
    auto oproblem = construct<NonLinearEvolutionProblem>(ctx, params);
    TFEL_TESTS_CHECK(isInvalid(oproblem));
  }
};

TFEL_TESTS_GENERATE_PROXY(UseMultiMaterialNonLinearIntegratorTest,
                          "UseMultiMaterialNonLinearIntegratorTest");
...
int main(int argc, char** argv) {
  ...
  if (parameters.parallel) {
    m.addXMLTestOutput("ParallelUseMultiMaterialNonLinearIntegratorTest-" +
                       std::to_string(mfem_mgis::getMPIsize()) + "-" +
                       std::to_string(mfem_mgis::getMPIrank()) + ".xml");
  } else {
    m.addXMLTestOutput("UseMultiMaterialNonLinearIntegratorTest.xml");
  }
