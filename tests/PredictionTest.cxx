...
template <bool parallel>
void export_prediction(mfem_mgis::FiniteElementDiscretization &fed,
...
int main(int argc, char *argv[]) {
  ...
  if (parallel) {
    ...
    export_prediction<true>(problem.getFiniteElementDiscretization(), du);
#else  /* MFEM_USE_MPI */
    return EXIT_FAILURE;
#endif /* MFEM_USE_MPI */
  } else {
    auto du = computePrediction<false>(ctx, problem);
    export_prediction<false>(problem.getFiniteElementDiscretization(), du);
  }
  return EXIT_SUCCESS;
}
